...
self.addEventListener('push', e => {
  ...
  const options = {
    ...
    data: { url: data.url || '/frota-app/empresa.html' },
  };
...
self.addEventListener('notificationclick', e => {
  ...
  const url = e.notification.data?.url || '/frota-app/empresa.html';
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(list => {
      for (const client of list) {
        if (client.url.includes('empresa.html') && 'focus' in client) return client.focus();
      }
